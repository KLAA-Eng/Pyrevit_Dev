...
def collect_placed_view_ids(document):
    placed_view_ids = set()
...
def build_view_note_map(target_views, matching_notes, placed_view_ids):
    target_view_dict = {}
    for v in target_views:
        key = get_elementid_value(v.Id)
        if key is not None:
    ...
    for note in matching_notes:
        ...
        # Include sheet-owned notes always.
        # Include view-owned notes only if the owner view is placed on a sheet.
        if view.ViewType != DB.ViewType.DrawingSheet and key not in placed_view_ids:
            continue
...
target_views = collect_target_views(doc)
matching_notes = collect_matching_textnotes(doc)
placed_view_ids = collect_placed_view_ids(doc)
...
view_note_map = build_view_note_map(target_views, matching_notes, placed_view_ids)

if not view_note_map:
    set_button_orange_not_hidden()
    forms.alert("No hideable matching engineer notes found on sheets or in views placed on sheets.", exitscript=True)
...
#     "Action: {0}\n"
#     "Views/Sheets processed: {1}\n"
#     "Notes {2} in views placed on sheets: {3}\n"
#     "Notes {2} on sheets: {4}"
# ).format(
#     action_label,
#     processed_views,
...
msg = (
    "Number of engineer's notes {0} in views placed on sheets: {1}\n"
    "Number of engineer's notes {0} on sheets: {2}"
).format(
    action_word,
    view_notes_changed,
